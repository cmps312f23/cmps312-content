...
import qu.lingosnacks.ui.editor.components.DefinitionList
import qu.lingosnacks.ui.editor.components.DefinitionsTab
import qu.lingosnacks.ui.editor.components.SentenceList
import qu.lingosnacks.ui.editor.components.SentencesTab
...
fun WordEditor(
    ...
    fun isUnique() = PackagesRepository.isUniqueWord(text, words.filterNot { it.text == word.text })
    fun requiredFilled() =
        text.isNotBlank() && sentences.isNotEmpty() && definitions.isNotEmpty() && isUnique()
    ...
    Dialog(
        ...
        Column(
            ...
            TabRow(selectedTab) {
                Tab(selectedTab == 0, { selectedTab = 0 }, text = { Text("Sentences") })
                Tab(selectedTab == 1, { selectedTab = 1 }, text = { Text("Definitions") })
            }
            if (selectedTab == 0) {
...
val types = listOf(ResourceTypeEnum.Photo, ResourceTypeEnum.Video, ResourceTypeEnum.Website)